#!/bin/sh
# RuxBoy macOS ビルドスクリプト
# 使い方: scripts/build_macos.sh [--debug|--release]
#
...
if ! sdl_prefix=$(pkg-config --variable=prefix sdl3 2>/dev/null); then
    echo "SDL3 が見つからない。'brew install sdl3' などで導入してください。" >&2
    exit 1
fi
...
bin="$dir/Packages/App/Bin/$profile/macOS/$archdir/RuxBoy"
echo "ビルド完了: $bin"
