...
- **コミット・プッシュは毎回、その場での明示的な指示があるまで行わない。** 「続けてください」「開発を進めて」等は実装作業の継続許可であり、コミット・プッシュの許可ではない。「コミットして」と言われた場合も、その時点でステージ済み・変更済みの内容に対する一回限りの許可として扱い、それ以降に発生した新しい変更には改めて確認する。複数の作業単位（例: 機能A実装→機能B実装→ドキュメント更新）にまたがる包括的な事前承認とは解釈しない。
- **理由**: `docs/dev` のリモート（`git@github.com:bubio/dev-docs.git`）・本体リポジトリのリモートはいずれもプライベートで、push すると GitHub Actions の CI が走り使用量（CI 分数）を消費する。無許可の push はこの消費を無断で発生させることになるため、特に慎重に扱う。
- 許可が出た後の push 先は、特別な指示がなければブランチを切らず `main`（`docs/dev` は `ruxboy` ブランチ）へ直接コミット・プッシュする。
